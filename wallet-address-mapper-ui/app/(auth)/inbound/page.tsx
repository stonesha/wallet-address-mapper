...
export default async function InboundPage() {
  ...
  const shipmentsRes: ShipmentResponse = await fetch(`${process.env.BACKEND_URL!}/getAllShipments/${address}/inbound`).then(res => res.json())

  return (
    <div className="grid grid-cols-3 gap-4">
      {shipmentsRes.shipments && shipmentsRes.shipments.map(shipment => <PackageCard key={shipment.shipment_id} {...shipment} />)}
      <PackageCard shipment_hash={"test"} status={2} from={"0x01234"} to={address} shipping_cost_usd="12" type={1} weight={1} length={1} width={2} height={2} />
